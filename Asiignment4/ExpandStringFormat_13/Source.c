...
int expand(char *s1, char *s2)
{
	...
	while (s1[i] != '\0')		//Running until end of string
	{
		if (s1[i] == '-')		//if expansion symbol is encountered 
		{
			...
			//printf("%c ", s1[i + 1]);
			if (s1[i+1] == '-')		//condition to check string of format a--h
				return -1;

				if ((isdigit(s1[i - 1]) && isalpha(s1[i + 1])) || (isdigit(s1[i + 1]) && isalpha(s1[i - 1])))		//if start and end characters of string are in invalid format
				{
					printf("You have entered input to be expand as alpha-digit or digit-alpha format\n");
					return 0;
				}
				else if (isalpha(s1[i - 1]) && isalpha(s1[i + 1]))		//if start and end charcters are alphabets
				{
					//printf("Hello");
					...
					else  if ((start >= 'a'&&end <= 'z') || (start >= 'A'&&end <= 'Z') )		//condition to check if two characters of string before and after '-' are in valid format
					{
				...
				else if (isdigit(s1[i - 1]) && isdigit(s1[i + 1]))		//Check condition for number expansion
				{
					if (start >= end)
					{
						printf("You have entered wrong order to expand\n");
					...
					else if (start >= '0'&&end <= '9')		//Expanding digits
					{
						for (j = start + 1; j < end; j++)
						{
							s2[k] = j;
...
int main()
{
	...
	/*----------------Taking String1 data-------------*/
	printf("Enter the size for the string that you want to expand(minimum size must 3): ");
